...
transformed parameters {
	...
	for(i in 1:N){
		y_hat[i] <- a[sp[i]] + 
			...
			b_chill2[sp[i]] * chill2[i]+ b_chill4[sp[i]] * chill4[i]+ b_chill8[sp[i]] * chill8[i] +
						b_time2[sp[i]] * time2[i] + b_time3[sp[i]] * time3[i]
		;
...
model {
	...
	b_chill8 ~ normal(mu_b_chill8, sigma_b_chill8);
	b_time2 ~ normal(mu_b_time2, sigma_b_time2);
	b_time3 ~ normal(mu_b_time3, sigma_b_time3);		
